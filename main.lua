...
--[[
    Custom function to test for individual keystrokes outside of the default 
    `love.keypressed` callback.
]]
function love.keyboard.wasPressed(key)
    -- Return true if the key was pressed this frame, otherwise return false
    return love.keyboard.keysPressed[key] == true or false
end
...
function love.draw()
    -- Begin drawing using push to handle virtual resolution
    push:apply('start')
    ...
    -- Draw the background, scaling to fit the screen
    love.graphics.draw(gTextures['background'], 0, 0, 0, VIRTUAL_WIDTH / (bgWidth - 1), VIRTUAL_HEIGHT / (bgHeight - 1))
    
    -- Defer rendering to the current state using the state machine
...
function loadHighScores()
    -- Set the identity for file operations
    love.filesystem.setIdentity('breakout')

    -- Initialize scores table
    local scores = {}

    -- Check if the file exists; if not, initialize it with default scores
    ...
    local file = love.filesystem.newFile('breakout.lst', 'r')
    file:open('r')
    ...
    for line in file:lines() do
        if name then
            -- Store the name in the scores table
            scores[counter] = {
...
function renderHealth(health)
    -- Initialize health rendering position
