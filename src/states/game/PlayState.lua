...
function PlayState:enter(params)
    ...
    self.levelComplete = params.levelComplete
    self.level = LevelMaker.generate(self.levelWidth or 100, 10)
    ...
    self.player = Player({
        ...
        map = self.tileMap,
        level = self.level,
        levelComplete = self.levelComplete or false
    })
...
function PlayState:update(dt)
    ...
    -- remove any nil references from pickups, etc.
    self.level:clear()
...
function PlayState:render()
    ...
    -- render level completion message if level is complete
