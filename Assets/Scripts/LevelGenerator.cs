...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				} else if (mapData[z,x] != Block.HOLE && !characterPlaced) {
	...
	Block[,] GenerateMazeData() {
		Block[,] data = new Block[mazeSize, mazeSize];
		...
		while (tilesConsumed < tilesToRemove) {
			...
			for (int i = 0; i < numSpacesMove; i++) {
				...
				if (data[mazeY, mazeX] == Block.WALL) {
					data[mazeY, mazeX] = Block.FLOOR;
					tilesConsumed++;
				}
			}
		}
